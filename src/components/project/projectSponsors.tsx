    ...
    return (
        <>
            {sponsors.map((sponsor) => <div className="flex mb-4 items-center bg-white rounded-md p-8 border-2 border-custom-gray">
                <img className="w-20 h-20 rounded-full shadow-lg" src={sponsor.avatar} />
                <div className="flex flex-col ml-6 ">
                    <span className="text-xl font-medium">{sponsor.name}</span>
                    <span className="flex text-2xl text-gray-400 mt-2">{"Внес " + sponsor.sum}
                        <div className="text-normal-text w-5 ml-0.5">
                            <CoinIcon />
                        </div>
                    </span>
                    <span className="mt-2">{sponsor.message}</span>
                </div>
